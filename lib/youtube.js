...
function getConfiguredCookies() {
  ...
  const rawCookies = encodedCookies ? decodeCookieEnvironment(encodedCookies) : process.env.YOUTUBE_COOKIES;
...
function decodeCookieEnvironment(value) {
...
function looksLikeRawCookies(value) {
  return (
...
export function getPublicError(error) {
  ...
  if (
    ...
    if (status.hasCookies) {
      return "YouTube bloqueo el servidor de Vercel incluso con cookies. Configura YOUTUBE_PROXY_URL o mueve el backend a un hosting con IP menos bloqueada.";
    }
