import { decodeCookieEnvironment, getRawCookieEnvironment, looksLikeRawCookies } from "./cookie-env.js";
...
function getConfiguredCookies() {
  const rawCookies = getRawCookieEnvironment();
...
function looksLikeNetscapeCookies(rawCookies) {
  return rawCookies
    ...
    .some((line) => {
      ...
      if (!trimmedLine || trimmedLine.startsWith("# Netscape")) return false;
      return getNetscapeCookieFields(trimmedLine).length >= 7;
    });
}
...
function parseNetscapeCookies(rawCookies) {
  return rawCookies
    ...
    .map((line) => {
      const fields = getNetscapeCookieFields(line);
...
function getNetscapeCookieFields(line) {
  const tabFields = line.split("\t");
  if (tabFields.length >= 7) return tabFields;
...
export function getPublicError(error) {
  ...
  if (
    ...
    const status = getCookieEnvironmentStatus();

    if (status.hasCookies) {
      return "YouTube bloqueo el servidor actual incluso con cookies. Configura YOUTUBE_PROXY_URL con un proxy confiable e intenta de nuevo.";
    }

    return "YouTube bloqueo la solicitud del servidor. Configura YOUTUBE_COOKIES_BASE64 e intenta de nuevo.";
  }
...
export function getCookieEnvironmentStatus() {
  const rawValue = process.env.YOUTUBE_COOKIES_BASE64 || process.env.YOUTUBE_COOKIES || "";
  const decodedValue = process.env.YOUTUBE_COOKIES_BASE64
    ? decodeCookieEnvironment(process.env.YOUTUBE_COOKIES_BASE64)
    : rawValue;
  const cookies = getConfiguredCookies();
  ...
  return {
    ...
    diagnostics: {
      envValueLength: rawValue.length,
      decodedLength: decodedValue.length,
      lineCount: decodedValue ? decodedValue.split(/\r?\n/).length : 0,
      hasNetscapeHeader: decodedValue.includes("Netscape HTTP Cookie File"),
      hasYoutubeDomain: decodedValue.includes("youtube.com"),
      parseMode: looksLikeNetscapeCookies(decodedValue)
        ? "netscape"
        : decodedValue.includes(";")
          ? "cookie-header"
          : decodedValue.trim().startsWith("[") || decodedValue.trim().startsWith("{")
            ? "json"
            : "unknown",
    },
